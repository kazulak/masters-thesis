...
def qualification_arms(spec, capacity):
    ...
    return [fe.up(d, t, policy=fe.F32) for d, t in sorted(resources)]


def fixture_cases():
    small = [
        dict(
            case_id='qual_' + fe.case_id(f, 5 if f == 'edc' else 4),
            family=f,
            n=5 if f == 'edc' else 4,
            layers=1,
            width_slot=4,
    ...
    for layers in (1, 2, 4, 8):
        small.append(
            dict(
                case_id=f'qual_stress_n04_l{layers}',
                family='stress',
                n=4,
                layers=layers,
                width_slot=4,
    ...
    for f in ('ghz', 'stress'):
        small.append(
            dict(
                case_id=f'qual_{f}_n06',
                family=f,
                n=6,
                layers=2 if f == 'stress' else 1,
                width_slot=6,
                role='resource_qualification',
            )
        )
    return small
...
def main():
    ...
    for c in fixtures:
        ...
        result = fe.call_worker(
            source, work, 'reference', cp, work / 'reference_records' / (cid + '.json'),
    ...
    for c in fixtures:
        for threads in sorted({1, len(b['cpu_cpus'])}):
            arm = dict(backend='quest32', threads=threads)
            ap = directory / (fe.arm_id(arm) + '.arm.json')
            if not ap.exists():
                write(ap, arm)
            result = fe.call_worker(
                source, work, 'cpu', work / 'case_records' / (c['case_id'] + '.json'),
                directory / (c['case_id'] + '__' + fe.arm_id(arm) + '.json'), arm=ap,
                cpus=b['cpu_cpus'] if threads > 1 else [b['cpu_cpus'][0]], quest32=b['quest32']
            )
            fe.require(
                result.get('status') == 'success' and result.get('accuracy_qualified') is True,
        ...
        with lockpath.open('a') as lock:
            ...
            for arm in arms:
                ...
                for c in fixtures[-2:]:
                    ...
                    r = fe.call_worker(
                        source, work, mode, work / 'case_records' / (c['case_id'] + '.json'), out,
    ...
    write(
        work / 'qualified.json',
        dict(
            schema='thesis_final_qualification_v1',
            passed=True,
            binding_sha256=fe.digest(work / 'binding.json'),
            physical_attempts=len(arms) * 2,
            simulator_attempts=len(arms) * 2,
            reference_cases=len(fixtures),
